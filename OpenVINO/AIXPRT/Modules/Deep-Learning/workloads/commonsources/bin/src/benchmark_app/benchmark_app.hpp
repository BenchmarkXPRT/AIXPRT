...
static const char model_name_message[] = "AIXPRT. Path to an .xml file with a trained model.";
DEFINE_string(aarch, "", "Architecture");
static const char architecture_message[] = "AIXPRT. Specify the target device to infer on; CPU, GPU, FPGA, HDDL or MYRIAD is acceptable. ";
DEFINE_string(prec, "", "Precision");
static const char precision_message[] = "AIXPRT. precision size (default 16)";
...
static void showUsage() {
    ...
    std::cout << "    -a \"<path>\"             " << model_name_message << std::endl; //AIXPRT
    std::cout << "    -aarch \"<string>\"       " << architecture_message << std::endl; //AIXPRT
    std::cout << "    -prec \"<integer>\"       " << precision_message << std::endl; //AIXPRT
}
