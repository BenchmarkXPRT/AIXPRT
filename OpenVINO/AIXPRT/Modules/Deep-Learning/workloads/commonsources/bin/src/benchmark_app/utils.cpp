...
uint32_t deviceDefaultRequestsNumber(const std::string& device) {
    static const std::map<std::string, uint32_t> deviceDefaultAsyncNiReq {
            { "CPU",     2    },
            { "GPU",     2    },
            { "VPU",     4    },
            { "MYRIAD",  4    },
    ...
    uint32_t nireq = 0;
    for (const auto& deviceAsyncNiReq : deviceDefaultAsyncNiReq) {
        if (device.find(deviceAsyncNiReq.first) != std::string::npos) {
            nireq = std::max(nireq, deviceAsyncNiReq.second);
        }
    }
    if (nireq == 0) {
        const auto unknownDeviceIt = find_if(
            deviceDefaultAsyncNiReq.begin(),
            deviceDefaultAsyncNiReq.end(),
            [](std::pair<std::string, uint32_t> deviceNireq) { return deviceNireq.first == "UNKNOWN"; });

        if (unknownDeviceIt == deviceDefaultAsyncNiReq.end()) {
            throw std::logic_error("UNKNOWN device was not found in the device duration list");
        }
        nireq = unknownDeviceIt->second;
        slog::warn << "Default number of requests " << nireq << " for unknown device '" << device << "' is used" << slog::endl;
    }
    return nireq;
}
